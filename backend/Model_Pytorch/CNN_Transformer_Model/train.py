...
def train_model(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for x_batch, y_batch in train_loader:
            ...
            # If y_batch is shape (batch, label_width, 1), flatten:
            # e.g. y_batch = y_batch[:, -1, 0], or y_batch.squeeze(-1)
            # Just be sure your shapes match.
            y_batch_single = y_batch.squeeze(-1).squeeze(-1)  # (batch,)
            # preds is (batch, 1), so also flatten:
